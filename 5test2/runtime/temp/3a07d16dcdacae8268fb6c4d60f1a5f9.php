...
<!-- 头部结束-->
	...
	<P>首页&nbsp>&nbsp<?php echo getCateName($art['cate_id']); ?>&nbsp>&nbsp<?php echo $art['title']; ?></P>
	<!-- 主体 -->
	<div class="row">
		<!-- 左侧8列 -->
		<div class="col-md-8">
		...
		<div class="page-header">
  			<h3><?php echo getCateName($art['cate_id']); ?></h3>			
		</div>

		<div style="width:%60">			
            
             <h4><?php echo $art['title']; ?></h4>
                <p>作者:<?php echo getUserName($art['user_id']); ?>&nbsp;&nbsp;
               发布时间:<?php echo $art['create_time']; ?>&nbsp;&nbsp;
               浏览:<?php echo $art['pv']; ?>&nbsp;&nbsp;
			...
			<div class="ContentItem-actions">
				...
				<button type="button" id="fav" user_id="<?php echo $art['user_id']; ?>" article_id="<?php echo $art['id']; ?>" session_id="<?php echo \think\Session::get('user_id'); ?>" class="Button ContentItem-action Button--plain Button--withIcon Button--withLabel">
					<span style="display: inline-flex; align-items: center;">
						​
				...
				<button type="button" id="like" user_id="<?php echo $art['user_id']; ?>" article_id="<?php echo $art['id']; ?>" session_id="<?php echo \think\Session::get('user_id'); ?>" class="Button ContentItem-action Button--plain Button--withIcon Button--withLabel">
					<span style="display: inline-flex; align-items: center;">
						​
				...
				<form style="margin-top:10px;display:none;"   method="post" id="user_comment"  >
						...
						<input type="hidden" name="user_id" value="<?php echo $art['user_id']; ?>">
						<input type="hidden" name="reply_id" value="<?php echo \think\Session::get('user_id'); ?>">
				...
				<div style="border:1px solid #DDDDDD;border-radius:5px; padding:10px 20px 0px 20px; margin:10px 0px 10px 0px;" <?php if(getUserComment($art['id']) == '0'): ?>class="hidden"<?php endif; ?>> 
					...
					<div ><p><?php echo getUserName($com['reply_id']); ?>&nbsp回复&nbsp<?php echo getUserName($com['user_id']); ?>&nbsp<?php echo getDateTime($com['create_time']); ?>  </p></div>
					<p><?php echo $com['user_comment']; ?></p>
					<p></p>
					<p><?php echo $com['id']; ?></p>
					...
					<form style="margin-top:10px;" method="post" class="reply_comment" name="" >
							...
							<input type="hidden" name="article_id" value="<?php echo $com['article_id']; ?>">
							<input type="hidden" name="user_id" value="<?php echo $com['reply_id']; ?>">
							<input type="hidden" name="reply_id" value="<?php echo \think\Session::get('user_id'); ?>">
							<input type="text" class="form-control "  value="" name="reply_comment" placeholder="写下你的评论">
						  </div>
						  
						  
						  <button type="button"  class="reply Button ContentItem-action1 Button--plain Button--withIcon Button--withLabel">
							<span style="display: inline-flex; align-items: center;">​
								<svg class="Zi Zi--Comment Button-zi" fill="currentColor" viewBox="0 0 24 24"
								width="1.2em" height="1.2em">
									<path d="M10.241 19.313a.97.97 0 0 0-.77.2 7.908 7.908 0 0 1-3.772 1.482.409.409 0 0 1-.38-.637 5.825 5.825 0 0 0 1.11-2.237.605.605 0 0 0-.227-.59A7.935 7.935 0 0 1 3 11.25C3 6.7 7.03 3 12 3s9 3.7 9 8.25-4.373 9.108-10.759 8.063z"
						  ...
						  <button type="button" id="<?php echo $com['id']; ?>" value="<?php echo $com['id']; ?>" onclick="reply_open()" class="reply_open Button  Button--plain Button--withIcon Button--withLabel">
							<span style="display: inline-flex; align-items: center;">​
								<svg class="Zi Zi--Comment Button-zi" fill="currentColor" viewBox="0 0 24 24"
								width="1.2em" height="1.2em">
									<path d="M10.241 19.313a.97.97 0 0 0-.77.2 7.908 7.908 0 0 1-3.772 1.482.409.409 0 0 1-.38-.637 5.825 5.825 0 0 0 1.11-2.237.605.605 0 0 0-.227-.59A7.935 7.935 0 0 1 3 11.25C3 6.7 7.03 3 12 3s9 3.7 9 8.25-4.373 9.108-10.759 8.063z"
